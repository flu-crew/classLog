...
def trainClassifier(inputFile, delim="|", col=1, percentFeatures = 0, kfolds = 0):
    ...
    if(percentFeatures > 0 and percentFeatures <= 1):
        ...
        importanceScores = importances[sortedImportances]
...
def getFeatures(classifierPickle):
    ...
    returned in the format of [Position]_[Acid] [Importance]. The position is 
    relative to the importance score. If feature selection was skipped, 
    importance volumn will have no values.

    Parameters
    ----------
    classifierPickle : CLICK FILE
        Path to an output pickle file, generated by the trainClassifier function.
    ...
    #Create blank feature list via numpy, number of undef x number of features
    features = list(importClassifier["features"])                               #89.3us
    importances = list(importClassifier["importances"])
    result = "\n".join("{} {}".format(x, y) for x, y in zip(features, importances))
    print(result)
    return
